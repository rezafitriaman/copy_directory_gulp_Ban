...
if(theme == 'Template1')
{
	//custom fix voor leaderboard
			...
			$('.logoWrapper').css({'width' : '150','bottom':'15px','left' : 'auto','top': '6px'});
			$('.copy1Wrapper').css('width','150px');
			
	
	if(zoom == 'zoomIn')
	{
		//left 150px fix voo rleaderboard
		$('.background1img').css({'transform' : 'scale(1)','left' : '0px'});
		$('.background1img').css({'top' : '0px'});
	}
	else if(zoom == 'zoomOut' )
	{
			$('.background1img').css({'transform' : 'scale('+zoomScale+')','left' : 155+'px','top': '-10px'});
	}
